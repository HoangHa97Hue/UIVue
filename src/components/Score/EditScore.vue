<template>
  <div class="container">
    ...
    <form method="post" @submit="checkForm">
      ...
      <div class="form-group m-3">
        ...
        <small id="emailHelp" class="form-text text-muted">
          ...
          <div class="error" v-if="!$v.scoreID.patternScID">Score ID must match Sc**** (* is number) pattern</div>
      ...
      <div class="form-group m-3">
        ...
        <small id="emailHelp" class="form-text text-muted">
          ...
          <!-- <div class="error" v-if="!$v.score.between">Score must be between {{ $v.score.$params.between.min }} and
            {{ $v.score.$params.between.max }}</div> -->
      ...
      <div class="form-group m-3">
        ...
        <input type="number" class="form-control" :value="semester" @change="setSemester($event.target.value)" />
        <small id="emailHelp" class="form-text text-muted">
      ...
      <small id="emailHelp" class="form-text text-muted">
        <span>Old DateExxam: {{ dateExam }}</span>
      </small>
      <div class="form-group m-3">
      ...
      <button class="btn btn-light border border-info" v-on:click="changeDisplayScoreList">Back</button>
    ...
    <small id="emailHelp" class="form-text text-muted">
      ...
      <div class="text-info">{{ statusEdit }}</div>
    </small>
  </div>
...
export default {
  ...
  async mounted() {
    await this.getScoreDetail(); // neu nhu ko de await thi sao   
    this.scoreID = this.scoreDetails.scoreID;
    console.log('ScoreID of scoreDetail: ' + this.scoreDetails.scoreID)
    this.studentID = this.scoreDetails.studentID;
    console.log('studentID of scoreDetail: ' + this.scoreDetails.studentID)
    this.subjectID = this.scoreDetails.subjectID;
    this.instructorID = this.scoreDetails.instructorID;
    this.score = this.scoreDetails.scoreReality;
    this.year = this.scoreDetails.year;
    this.semester = this.scoreDetails.semester;
    this.dateExam = this.scoreDetails.dateExam; // xem lại chỗ này có cần hiển thị cái cũ ra trên span rồi input cái mới
  ...
  methods: {
    ...
    changeDisplayScoreList() {
      console.log("In Edit");
      this.$emit("changeDisplayScoreList", 0);
    },
    async getScoreDetail() {
      await axios
        .get(
          "https://localhost:44319/api/Score/GetScore/" +
          this.scoreIDForEdit,
          this.getTokenConfig()
        )
        .then(
          (response) =>
            (this.scoreDetails = JSON.parse(JSON.stringify(response.data)))
        );
    },
    checkForm(e) {
      ...
      if (this.submitStatus != 'Error') {
        let options = {
          ...
          url: 'https://localhost:44319/api/Score/EditScore',
        }
        ...
        axios(options)
          .then(response => {
            // this.error.push('Edit Score Succesfully. Please comeback to the list scores!');
            this.statusEdit = 'Edit Score Succesfully. Please comeback to the list scores!';
