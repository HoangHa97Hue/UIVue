<template>
    <div class="container mt-4">
        <RegisterComponentSignUp v-if="selectDisplayRegisterPageSignUp"
            v-on:changeDisplayRegisTerOnLogin="changeDisplayRegister($event)"></RegisterComponentSignUp>
        <form v-else method="post" @submit="checkForm">
            <h1>Login</h1>
            <div class="text-warning" style="width: 100%;text-align: center;">
                You should login with Account recently registered!!!</div>
            <div class="form-group m-3">
                ...
                <small id="emailHelp" class="form-text text-muted">
                    ...
                    <div class="error" v-if="!$v.email.email">Email must match {{ $v.email.$params.email }} pattern
                    </div>
...
import RegisterComponentSignUp from "../Login/RegisterComponentSignUp.vue";
...
export default {
    data() {
        return {
            ...
            error: [],
            submitStatus: null,
            selectDisplayRegisterPageSignUp: true,
            // disableButton: false,
        }
    },
    validations: {
    ...
    components: {
        RegisterComponentSignUp
    },
    methods: {
        getUserID() {
            var userIDRegister = localStorage.getItem('userID');
            return userIDRegister;
        },
        signUp() {
            this.$router.push({ name: 'Login_Component_SignUp' })//xius qua router dky thang Register
        },
        setEmail(value) {
            this.email = value,
                this.$v.email.$touch();
        ...
        checkForm(e) {
            //check thu userID co nhap vao giong thang khi lam o Register Signup ko
            var userIDRegister = this.getUserID();
            ...
            axios(options)
                .then(response => {
                    ...
                    console.log('response.data.userID:' + JSON.parse(JSON.stringify(response.data)).userID);
                    console.log('userIDRegister:' + userIDRegister);
                    if (JSON.stringify(JSON.parse(JSON.stringify(response.data)).userID) === userIDRegister) {
                        this.$router.push({ name: 'Create_Student_Register_SignUp' });
                    }
                    ...
                    // });//chuyeenr dden trang list Department
        ...
        changeDisplayRegister(value) {
            this.selectDisplayRegisterPageSignUp = value;
            console(' this.selectDisplayRegisterPageSignUp' + this.selectDisplayRegisterPageSignUp);
        }
