<template>
    <div class="container">
        ...
        <form method="post" @submit="checkForm">
            <h1>Enroll Subject For Student</h1>
             <small id="emailHelp" >
                    <div class="text-info">Student Account Must update completely private's information before Enroll Subject</div>
                    <div class="text-info">Student only enrolls less than or equals 2 times for each Subject</div>
                </small>
            <small id="emailHelp" class="form-text text-muted" style="color: red !important;">
                {{ errorOrMessageFromBE }}
            </small>
            <div class="form-group m-3">
                <label class="form__label">ID Enroll </label>
                ...
                <small id="emailHelp" class="form-text text-muted">
                    <div class="error" v-if="!$v.scoreID.required"> ID is required</div>
                    <div class="error" v-if="!$v.scoreID.patternScID"> ID must match Sc**** (* is number) pattern
                    </div>
                    <div class="error" v-if="!$v.scoreID.maxLength"> ID must have at most
            ...
            <div class="row form-group d-flex m-3">
                <div class="col-4">
                    ...
                    <select disabled v-model="studentID" id="selectStudent">
            ...
            <button style="margin-left: 10px;" class="btn btn-secondary back_btn">
        ...
        <small id="emailHelp" class="form-text text-muted">
            ...
            <div class="text-info"> {{enrollStatus}}</div>
        </small>
    </div>
...
export default {
    ...
    mounted(){
         axios.get('https://localhost:44319/api/Student/GetStudentByAspUserID/' + this.getUserID(), this.getTokenConfig())
                .then((response) =>(
                    console.log(response.data),
                    this.studentID = JSON.parse(JSON.stringify(response.data)).id));
    ...
    methods: {
        ...
        checkForm(e) {
            ...
            axios(options)
                .then(response => {
                    this.enrollStatus = 'Enroll SubjectID: successfully',
                    this.errorOrMessageFromBE = null,
                    console.log('eroll status:'+this.enrollStatus);
