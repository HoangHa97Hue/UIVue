<!-- <template>
    <div>
        ...
        <!-- ko can load lai vi du sao thang login no cung bat load lai a-->
        <LoginComponent v-else-if="isFromRegisterPage" v-on:changeDisplayRegisTerOnCreate="changeDisplayRegister($event)">
        </LoginComponent>
        <!-- v-on:reloadPageCreateStudent="reloadCreateStudentPage()-->

        <div v-else><!--v-if="!selectDisplayRegisterPage && !isFromRegisterPage"-->
...
export default {
    data() {
        return {
            ...
            isFromRegisterPage: null,// bien nay theo doi no co tu register page ko,
            role: null
        };
    },
    validations: {
    ...
    },//co can phai them thang watch de theo doi thang  fromRegisterPage no thay doi ko ????????????????
    watch: {
        isFromRegisterPage() {
            this.fetchGradeList();
        }
    ...
    mounted() {
        window.addEventListener('fromRegisterPage-localstorage-changed', (event) => {
            this.isFromRegisterPage = event.detail.storage;
        });
    ...
    created() {
        ...
        //XEM LAI CHO CHỖ NÀY THẤY SAI SAI
